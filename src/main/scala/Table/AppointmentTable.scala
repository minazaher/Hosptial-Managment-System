...
class AppointmentTable(tag: Tag) extends Table[Appointment](tag, "Appointments") {
  def appointment_id = column[Int]("appointment_id", O.PrimaryKey, O.AutoInc)
  ...
  def approved_by = column[Int]("approved_by")

  override def * = (appointment_id, doctor_id, patient_id, appointment_date_time, appointment_purpose, status, approved_by) <> (Appointment.tupled, Appointment.unapply)
